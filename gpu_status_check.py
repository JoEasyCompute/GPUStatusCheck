...
trap 'rm -f "$tmp_out" "$tmp_err" "$tmp_log" "$tmp_pmon" "$tmp_telemetry"' EXIT INT TERM
...
if [ "$gpu_count" -gt 0 ] 2>/dev/null && command -v nvidia-smi >/dev/null 2>&1; then
    if nvidia-smi pmon -c 1 >"$tmp_pmon" 2>/dev/null; then
...
gpu_avg_temp_c=""
if [ "$gpu_count" -gt 0 ] 2>/dev/null && command -v nvidia-smi >/dev/null 2>&1; then
    if nvidia-smi --query-gpu=power.draw,temperature.gpu --format=csv,noheader,nounits >"$tmp_telemetry" 2>/dev/null; then
        telemetry=$(awk -F',' '
            function trim(value) {
            ...
            END {
                if (power_seen) {
                    printf "%.1f", power_total
                }
                printf "|"
                if (temp_count > 0) {
                    printf "%.1f", temp_total / temp_count
                }
            }
        ' "$tmp_telemetry")
